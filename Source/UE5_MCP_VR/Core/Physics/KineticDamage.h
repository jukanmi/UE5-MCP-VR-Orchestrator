...
/**
 * 동역학 데미지 공용 헬퍼 — 근접 스윙(VRPawn)·투사체(KineticProjectile)·NPC/적 공격 판정 단일 소스.
 * 운동에너지 ½mv² 를 HP 데미지로 환산하고, 부위 인지·래그돌 임펄스를 위해
 * FPointDamageEvent(본 이름·발사 방향 포함)로 전투 캐릭터(SmartNPC·EnemyCharacter)에 전달한다.
 */
namespace KineticDamage
{
    ...
    /**
     * FPointDamageEvent 로 전투 캐릭터에 데미지 적용. 충돌점 최근접 본을 BoneName 에 채워
     * 부위 배율·래그돌 임펄스(ShotDirection)가 동작하게 한다.
     * BaseHit 제공 시 그 HitInfo 를 유지(투사체 충돌 결과), nullptr 이면 ImpactPoint 만 채운다(근접).
     */
    void ApplyToNPC(ACombatCharacter* NPC, float Damage, const FVector& ImpactPoint,
