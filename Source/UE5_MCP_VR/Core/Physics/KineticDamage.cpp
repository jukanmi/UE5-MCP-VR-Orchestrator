...
void KineticDamage::ApplyToNPC(ACombatCharacter* NPC, float Damage, const FVector& ImpactPoint,
    const FVector& ShotDirection, AController* InstigatorController, AActor* Causer,
    const FHitResult* BaseHit)
{
    if (!NPC) return;

    // FPointDamageEvent 로 보내야 피격자가 BoneName(부위 배율)·ShotDirection(래그돌 임펄스)을 처리.
    ...
    // 실패 시 None→Torso 폴백(ACombatCharacter::BodyPartMultiplierForBone).
