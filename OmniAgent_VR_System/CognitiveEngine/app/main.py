...
async def websocket_llm_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        if STATE.active_llm_ws is websocket:
            STATE.active_llm_ws = None
...
async def _handle_state_update(envelope: MessageEnvelope) -> str:
    try:
        ...
        async with STATE.world_state_lock:
            STATE.cached_world_states[state_payload.owner_agent_id.lower()] = state_payload.model_dump()
        logger.debug(
...
# ─────────────────────────────────────────────────────────────────────────────
# 디버그 대시보드·REST — debug_routes.py. _handle_prompt 만 넘겨 준다(순환 import 회피).
# ─────────────────────────────────────────────────────────────────────────────
app.include_router(build_router(_handle_prompt))
