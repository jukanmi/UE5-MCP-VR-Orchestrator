...
# 바닥에 놓였을 때 눈에 들어오는 하한. 표에는 현실 치수를 남기고 적용할 때만 끌어올린다.
#
# 한때 15 로 뒀는데 그건 오진이었다. 아이템이 손톱만하게 보인 진짜 원인은 치수가 아니라
# BP_DropItem 의 액터 스케일 0.3 × ItemMesh 컴포넌트 스케일 0.3 = 0.09 배였다(기본 큐브
# 100cm 를 폴백으로 쓰던 시절의 잔재). 스케일을 1.0 으로 되돌린 뒤에는 현실 치수로도
# 충분히 보여서, 동전이 접시만 해지는 하한을 걷어냈다.
MIN_VISIBLE_CM = 4
